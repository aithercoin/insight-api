...
BlockController.prototype._normalizePrevHash = function(hash) {
  // TODO fix aither to give back null instead of null hash
...
BlockController.prototype._getBlockSummary = function(hash, moreTimestamp, next) {
  ...
  if (summaryCache) {
    ...
    self.node.services.dashd.getRawBlock(hash, function(err, blockBuffer) {
      ...
      var br = new aither.encoding.BufferReader(blockBuffer);
      ...
      // would require changes in aither-node.
      var header = aither.BlockHeader.fromBufferReader(br);
      var info = {};
      var txlength = br.readVarintNum();
      info.transactions = [aither.Transaction().fromBufferReader(br)];
