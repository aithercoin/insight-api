...
var aither = require('@aithercore/aither-lib');
...
describe('Transactions', function() {
  ...
  describe('/rawtx/:txid', function() {
    it('should give the correct data', function(done) {
      ...
      var node = {
        getTransaction: sinon.stub().callsArgWith(1, null, aither.Transaction().fromBuffer(new Buffer(hex, 'hex')))
      };
  ...
  describe('#transformInvTransaction', function() {
    it('should give the correct data', function() {
      ...
      var tx = aither.Transaction().fromBuffer(new Buffer(rawTx, 'hex'));

      var node = {
        network: aither.Networks.livenet
      };
    ...
    it('will not include null values in vout array', function() {
      ...
      var node = {
        network: aither.Networks.testnet
      };
    ...
    it('should detect RBF txs', function() {
      ...
      _.each(testCases, function(tc) {
        var tx = aither.Transaction().fromBuffer(new Buffer(tc.rawTx, 'hex'));
        var result = transactions.transformInvTransaction(tx);
